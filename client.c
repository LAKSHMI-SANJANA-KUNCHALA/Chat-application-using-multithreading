...
character
 ...
 pthread_create(&receive_thread, NULL, receive_messages,
NULL);
...
"[%s] %s", username, message);
 send(client_socket, formatted_message,
strlen(formatted_message), 0);
 }
 // Notify the server that this client is exiting
 char exit_message[MAX_MSG_SIZE];
